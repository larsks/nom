...
// TODO: pagination
func (sls SQLiteStore) GetAllItems(ordering string) ([]store.Item, error) {
	itemStmt := `
	...
	if err != nil {
		return []store.Item{}, fmt.Errorf("store.go: GetAllItems: %w", err)
	}
	defer rows.Close()
...
func (sls SQLiteStore) GetItemByID(ID int) (store.Item, error) {
	var stmt *sql.Stmt
	...
	if err != nil {
		return store.Item{}, fmt.Errorf("[store.go] GetItemByID: %w", err)
	}
